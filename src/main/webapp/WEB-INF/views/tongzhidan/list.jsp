        ...
        <div class="col-md-9">
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    <form class="form-horizontal"action="${pageContext.request.contextPath}/tongzhidan/list" method="post">
                        ...
                        <div class="form-group">
                            <label for="inputEmail3" class="col-sm-2 control-label">通知单收入编码</label>
                            <div class="col-sm-4">
                                <select name="feeCode" id="feeCode" class="form-control">
                                    <option value="">请选择...</option>
                                    <!--导入核心标签库 item:控制层的属性名；>:显示-->
                                    <%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c" %>
                                    <c:forEach items="${feeList}" var="fee">
                                        <option value="${fee.businessFeeTypeCode}">${fee.businessFeeTypeCode}</option>
                                    </c:forEach>
                                </select>
                            </div>
                        ...
                        <div class="panel-body">
                            <form class="form-horizontal">
                                <table class="table table-bordered table-hover table-striped">
                                    ...
                                    <c:forEach items="${rpBusinessFeeRecordList}" var="rpBusinessFeeRecord">
                                        <tr>
                                            <td><input type="checkbox"></td>
                                            <td>${rpBusinessFeeRecord.id}</td>
                                            <td>${rpBusinessFeeRecord.cityCode}</td>
                                            <td>${rpBusinessFeeRecord.productCode}</td>
                                            <td>${rpBusinessFeeRecord.businessTypeCode}</td>
                                            <td>${rpBusinessFeeRecord.businessFee}</td>
                                            <td><fmt:formatDate pattern="yyyy-MM-dd"
                                                                value="${rpBusinessFeeRecord.businessRecordDate}" /></td>
                                            <td><a href="#">编辑</a></td>
                                            <td><a href="#">删除</a></td>
                                        </tr>
                                    </c:forEach>
